CREATE OR ALTER PROCEDURE CLIENT_MANAGER.CREATE_CLIENT
(
	@CLIENT_NAME VARCHAR(50),
	@CLIENT_SURNAME VARCHAR(50),
	@GENDER BIT,
	@MANAGER_ID BIT,
	@CLIENT_ID INT OUTPUT
)
AS BEGIN

	INSERT INTO CLIENT_MANAGER.CLIENTS(CLIENT_SURNAME, CLIENT_NAME, GENDER, MANAGER_ID)
	VALUES (@CLIENT_SURNAME, @CLIENT_NAME, @GENDER, @MANAGER_ID)

	SELECT @CLIENT_ID = SCOPE_IDENTITY();

END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.GET_USER_CLIENTS
(
	@USER_ID VARCHAR(50),
	@PAGE_NO INT,
	@ITEMS_PER_PAGE INT
)
AS BEGIN
	SELECT @PAGE_NO = @PAGE_NO - 1

	SELECT
		C.ID,
		C.CLIENT_NAME,
		C.CLIENT_SURNAME,
		C.GENDER,
		CASE
			WHEN SUM(A.BALANCE) > 5000
		THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END AS IS_VIP,
		U.USER_LOGIN
	FROM CLIENT_MANAGER.CLIENTS AS C
	LEFT JOIN CLIENT_MANAGER.CLIENT_ACCOUNTS AS A
		ON C.ID = A.CLIENT_ID
	JOIN CLIENT_MANAGER.USERS AS U
		ON C.MANAGER_ID = U.ID
	WHERE MANAGER_ID = @USER_ID
		OR C.MANAGER_ID IN (SELECT MANAGER_USER_ID FROM CLIENT_MANAGER.MANAGERS WHERE SUPER_MANAGER_ID = @USER_ID)
	GROUP BY C.ID,
		C.CLIENT_NAME,
		C.CLIENT_SURNAME,
		C.GENDER,
		U.USER_LOGIN
	ORDER BY C.ID
	OFFSET @PAGE_NO * @ITEMS_PER_PAGE ROWS
	FETCH NEXT @ITEMS_PER_PAGE ROWS ONLY
END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.GET_CLIENT_ACCOUNTS
	@CLIENT_ID INT
AS BEGIN
	SELECT
		ID,
		CODE,
		BALANCE
	FROM CLIENT_MANAGER.CLIENT_ACCOUNTS
	WHERE CLIENT_ID = @CLIENT_ID
END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.UPDATE_CLIENT_INFO
	@CLIENT_ID INT,
	@CLIENT_NAME NVARCHAR(50),
	@CLIENT_SURNAME NVARCHAR(50)
AS BEGIN
	UPDATE CLIENT_MANAGER.CLIENTS
		SET CLIENT_NAME = @CLIENT_NAME,
		CLIENT_SURNAME = @CLIENT_SURNAME
	WHERE ID = @CLIENT_ID
END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.GET_CLIENT_BY_ACCOUNT
(
	@ACCOUNT_CODE VARCHAR(10)
)
AS BEGIN

	SELECT
		C.ID,
		C.CLIENT_NAME,
		C.CLIENT_SURNAME
	FROM CLIENT_MANAGER.CLIENT_ACCOUNTS AS A
		JOIN CLIENT_MANAGER.CLIENTS AS C
		ON A.CLIENT_ID = C.ID
	WHERE A.CODE = @ACCOUNT_CODE

END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.CREATE_TRANSACTION
	@CLIENT_FROM INT,
	@CLIENT_TO INT,
	@ACCOUNT_FROM NVARCHAR(10),
	@ACCOUNT_TO NVARCHAR(10),
	@AMOUNT DECIMAL(20, 4),
	@IS_IMIDIATE BIT,
	@TRANSFER_DATE_TIME DATETIME OUTPUT
AS BEGIN

	DECLARE @TRANSACTION_ID INT

	INSERT INTO CLIENT_MANAGER.TRANSACTIONS(CLIENT_FROM, CLIENT_TO, ACCOUNT_FROM, ACCOUNT_TO, AMOUNT, CREATION_DATE_TIME, TRANSFER_DATE_TIME)
	VALUES (@CLIENT_FROM, @CLIENT_TO, @ACCOUNT_FROM, @ACCOUNT_TO, @AMOUNT, GETDATE(), CASE @IS_IMIDIATE WHEN 1 THEN GETDATE() ELSE NULL END)

	SELECT @TRANSACTION_ID = SCOPE_IDENTITY()

	IF @IS_IMIDIATE = 1
		EXEC CLIENT_MANAGER.CONDUCT_TRANSACTION @TRANSACTION_ID, @TRANSFER_DATE_TIME

END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.CONDUCT_TRANSACTION
	@TRANSACTION_ID INT,
	@TRANSFER_DATE_TIME DATETIME OUTPUT
AS BEGIN

	DECLARE
		@ACCOUNT_FROM NVARCHAR(10),
		@ACCOUNT_TO NVARCHAR(10),
		@AMOUNT NUMERIC(20, 4)

	SELECT
		@ACCOUNT_FROM = ACCOUNT_FROM,
		@ACCOUNT_TO = ACCOUNT_TO,
		@AMOUNT = AMOUNT
	FROM CLIENT_MANAGER.TRANSACTIONS
	WHERE ID = @TRANSACTION_ID
	AND TRANSFER_DATE_TIME IS NULL

	IF @@ROWCOUNT < 1
	RETURN

	UPDATE CLIENT_MANAGER.CLIENT_ACCOUNTS
	SET BALANCE = BALANCE - @AMOUNT
	WHERE CODE = @ACCOUNT_FROM
		AND BALANCE - @AMOUNT >= 0

	IF @@ROWCOUNT > 0
	BEGIN
		UPDATE CLIENT_MANAGER.CLIENT_ACCOUNTS
		SET BALANCE = BALANCE + @AMOUNT
		WHERE CODE = @ACCOUNT_TO

		SELECT @TRANSFER_DATE_TIME = GETDATE()

		UPDATE CLIENT_MANAGER.TRANSACTIONS
		SET TRANSFER_DATE_TIME = @TRANSFER_DATE_TIME
		WHERE ID = @TRANSACTION_ID
	END

END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.GET_TRANSACTIONS_BY_FILTER
(
	@ACCOUNT_FROM NVARCHAR(10) NULL,
	@ACCOUNT_TO NVARCHAR(10) NULL
)
AS BEGIN

	SELECT
		T.ID,
		T.CLIENT_FROM,
		T.CLIENT_TO,
		C_FROM.CLIENT_SURNAME AS CLIENT_SURNAME_FROM,
		C_FROM.CLIENT_NAME AS CLIENT_NAME_FROM,
		C_TO.CLIENT_SURNAME AS CLIENT_SURNAME_TO,
		C_TO.CLIENT_NAME AS CLIENT_NAME_TO,
		T.ACCOUNT_FROM,
		T.ACCOUNT_TO,
		T.AMOUNT,
		T.CREATION_DATE_TIME,
		T.TRANSFER_DATE_TIME
	FROM CLIENT_MANAGER.TRANSACTIONS AS T
	JOIN CLIENT_MANAGER.CLIENTS AS C_FROM
		ON T.CLIENT_FROM = C_FROM.ID
	JOIN CLIENT_MANAGER.CLIENTS AS C_TO
		ON T.CLIENT_TO = C_TO.ID
	WHERE T.ACCOUNT_FROM = ISNULL(@ACCOUNT_FROM, ACCOUNT_FROM)
		AND T.ACCOUNT_TO = ISNULL(@ACCOUNT_TO, ACCOUNT_TO)
	ORDER BY ID DESC

END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.CREATE_ACCOUNT
	@CLIENT_ID INT,
	@ACCOUNT_CODE NVARCHAR(10)
AS BEGIN
	INSERT INTO CLIENT_MANAGER.CLIENT_ACCOUNTS(CLIENT_ID, CODE, BALANCE)
	VALUES (@CLIENT_ID, @ACCOUNT_CODE, 0)
END;
GO

CREATE OR ALTER PROCEDURE CLIENT_MANAGER.GET_DATA_FOR_ACCOUNTS_REPORT
	@ACCOUNT_CODES NVARCHAR(MAX)
AS BEGIN
	
	SELECT
		A.CODE,
		A.BALANCE
	FROM CLIENT_MANAGER.CLIENT_ACCOUNTS AS A
	WHERE A.CODE IN (
		SELECT VALUE FROM string_split(@ACCOUNT_CODES, ',')
	)
END;
GO